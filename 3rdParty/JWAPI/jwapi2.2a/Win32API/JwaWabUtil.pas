...
{$IFNDEF DYNAMIC_LINK}
function CreateTable; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'CreateTable@36';
function MAPIInitIdle; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'MAPIInitIdle@4';
procedure MAPIDeinitIdle; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'MAPIDeinitIdle@0';
function FtgRegisterIdleRoutine; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'FtgRegisterIdleRoutine@20';
procedure DeregisterIdleRoutine; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'DeregisterIdleRoutine@4';
procedure EnableIdleRoutine; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'EnableIdleRoutine@8';
procedure ChangeIdleRoutine; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'ChangeIdleRoutine@28';
function MAPIGetDefaultMalloc; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'MAPIGetDefaultMalloc@0';
function OpenStreamOnFile; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'OpenStreamOnFile';
function PropCopyMore; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'PropCopyMore@16';
function UlPropSize; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'UlPropSize@4';
function FEqualNames; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'FEqualNames@8';
//procedure GetInstance; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'GetInstance';
function FPropContainsProp; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'FPropContainsProp@12';
function FPropCompareProp; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'FPropCompareProp@12';
function LPropCompareProp; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'LPropCompareProp@8';
function HrAddColumns; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'HrAddColumns@16';
function HrAddColumnsEx; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'HrAddColumnsEx@20';
procedure HrAllocAdviseSink; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'HrAllocAdviseSink@12';
procedure HrThisThreadAdviseSink; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'HrThisThreadAdviseSink@8';
function HrDispatchNotifications; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'HrDispatchNotifications@4';
procedure BuildDisplayTable; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'BuildDisplayTable@40';
function ScCountNotifications; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'ScCountNotifications@12';
function ScCopyNotifications; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'ScCopyNotifications@16';
function ScRelocNotifications; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'ScRelocNotifications@20';
function ScCountProps; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'ScCountProps@12';
function LpValFindProp; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'LpValFindProp@12';
function ScCopyProps; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'ScCopyProps@16';
function ScRelocProps; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'ScRelocProps@20';
function ScDupPropset; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'ScDupPropset@16';
function UlAddRef; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'UlAddRef@4';
function UlRelease; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'UlRelease@4';
procedure HrGetOneProp; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'HrGetOneProp@12';
procedure HrSetOneProp; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'HrSetOneProp@8';
function FPropExists; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'FPropExists@8';
function PpropFindProp; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'PpropFindProp@12';
procedure FreePadrlist; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'FreePadrlist@4';
procedure FreeProws; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'FreeProws@4';
function HrQueryAllRows; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'HrQueryAllRows@24';
function ScUNCFromLocalPath; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'ScUNCFromLocalPath@12';
function ScLocalPathFromUNC; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'ScLocalPathFromUNC@12';
function FtAddFt; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'FtAddFt@16';
function FtMulDwDw; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'FtMulDwDw@8';
function FtMulDw; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'FtMulDw@12';
function FtSubFt; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'FtSubFt@16';
function FtNegFt; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'FtNegFt@8';
function ScCreateConversationIndex; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'ScCreateConversationIndex@16';
procedure WrapStoreEntryID; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'WrapStoreEntryID@24';
function RTFSync; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'RTFSync';
function WrapCompressedRTFStream; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'WrapCompressedRTFStream';
function HrIStorageFromStream; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'HrIStorageFromStream@16';
function ScInitMapiUtil; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'ScInitMapiUtil@4';
procedure DeinitMapiUtil; external mapi32 {$IFDEF DELAYED_LOADING}delayed{$ENDIF} name 'DeinitMapiUtil@0';
{$ELSE}
